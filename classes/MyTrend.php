...
class MyTrend {
	...
	if($mysql_id) {
		...
		$sql = "SELECT * FROM `mytrend_mysql_instances` where group_name in('$allowedGroup') order by group_name,name";
    ...
    public function getServerGroup($ROLE) {
	$sql = "SELECT distinct group_name from `mytrend_mysql_instances`";
	$result1 = $this->objDBConnection->queryPDO($sql,array(),array());
	$allGroups = array();
	foreach($result1 as $res) {
	    $allGroups[] = $res['group_name'];	
	}
	...
	$allowedGroups = array_intersect($allGroups,$assignedGroups);
	if(!$allowedGroups) {
	    return $allGroups;
	}
	sort($allowedGroups);
